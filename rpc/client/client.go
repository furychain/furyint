...
var (
	// ErrConsensusStateNotAvailable is returned because Furyint doesn't use Tendermint consensus.
	ErrConsensusStateNotAvailable = errors.New("consensus state not available in Furyint")
)
...
//
// This is the type that is used in communication between cosmos-sdk app and Furyint.
type Client struct {
	*types.EventBus
...
// DumpConsensusState always returns error as there is no consensus state in Furyint.
...
// ConsensusState always returns error as there is no consensus state in Furyint.
