...
import (
	...
	cfg "github.com/xblackfury/furyint/config"
	"github.com/xblackfury/furyint/conv"
	"github.com/xblackfury/furyint/node"
	"github.com/xblackfury/furyint/rpc"
...
// These are exposed for convenience of commands embedding a furyint node
...
func NewRunNodeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:   "Run the furyint node",
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := checkGenesisHash(tmconfig); err != nil {
				return err
			}
			logger := log.NewTMLogger(log.NewSyncWriter(os.Stdout))
			err := startInProcess(&furyconfig, tmconfig, logger)
			if err != nil {
				return err
			}
			return nil
...
func startInProcess(config *cfg.NodeConfig, tmConfig *tmcfg.Config, logger log.Logger) error {
	...
	logger.Info("starting node with ABCI furyint in-process")
	...
	server := rpc.NewServer(furyintNode, tmConfig.RPC, logger)
	err = server.Start()
	if err != nil {
		return err
	}

	logger.Debug("initialization: furyint node created")
	if err := furyintNode.Start(); err != nil {
		return err
	}

	logger.Info("Started furyint node")

	// Stop upon receiving SIGTERM or CTRL-C.
	tmos.TrapSignal(logger, func() {
		if furyintNode.IsRunning() {
			if err := furyintNode.Stop(); err != nil {
