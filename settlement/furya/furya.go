...
func (dlc *LayerClient) Init(config []byte, pubsub *pubsub.Server, logger log.Logger, options ...settlement.Option) error {
	FuryaCosmosClient, err := newFuryaHubClient(config, pubsub, logger)
	if err != nil {
		return err
	}
	baseOptions := []settlement.Option{
		settlement.WithHubClient(FuryaCosmosClient),
	}
	if options == nil {
		options = baseOptions
	} else {
...
// PostBatchResp is the response after posting a batch to the Furya Hub.
...
// HubClient is the client for the Furya Hub.
...
func newFuryaHubClient(config []byte, pubsub *pubsub.Server, logger log.Logger, options ...Option) (*HubClient, error) {
	...
	for _, option := range options {
		option(furyesionHubClient)
	}

	if furyesionHubClient.client == nil {
		...
		furyesionHubClient.client = NewCosmosClient(client)
	}
	furyesionHubClient.rollappQueryClient = furyesionHubClient.client.GetRollappClient()
	furyesionHubClient.sequencerQueryClient = furyesionHubClient.client.GetSequencerClient()

	return furyesionHubClient, nil
}
...
// PostBatch posts a batch to the Furya Hub.
func (d *HubClient) PostBatch(batch *types.Batch, daClient da.Client, daResult *da.ResultSubmitBatch) (settlement.PostBatchResp, error) {
	...
	txResp, err := d.client.BroadcastTx(d.config.FuryAccountName, msgUpdateState)
...
// GetLatestBatch returns the latest batch from the Furya Hub.
...
// GetBatchAtIndex returns the batch at the given index from the Furya Hub.
...
func (d *HubClient) GetSequencers(rollappID string) ([]*types.Sequencer, error) {
	...
	for _, sequencer := range sequencers.SequencerInfoList {
		...
		err := d.protoCodec.UnpackAny(sequencer.Sequencer.FuryintPubKey, &pubKey)
		if err != nil {
			return nil, err
		}
...
func (d *HubClient) eventHandler() {
	...
	eventsChannel, err := d.client.SubscribeToEvents(context.Background(), "furya-client", fmt.Sprintf(eventStateUpdate, d.config.RollappID))
	if err != nil {
...
func (d *HubClient) convertBatchToMsgUpdateState(batch *types.Batch, daClient da.Client, daResult *da.ResultSubmitBatch) (*rollapptypes.MsgUpdateState, error) {
	account, err := d.client.GetAccount(d.config.FuryAccountName)
	if err != nil {
		return nil, err
	}
